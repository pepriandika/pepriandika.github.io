...
export async function generateStaticParams() {
  const posts = getAllPostSlugs("docs");
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const postData = await getPostData("docs", slug);
  return {
    title: `${postData.title} | Docs`,
    description: postData.description,
  };
}

export default async function DocsPost({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const postData = await getPostData("docs", slug);
